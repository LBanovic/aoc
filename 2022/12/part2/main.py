...
def dijkstra(start_state, map_matrix):
    vertices = []
    ...
    candidate_vertices = list(filter(lambda x: map_matrix[x[1]][x[0]] == 'a', vertices))
    ...
    return min(distances[c] for c in candidate_vertices)
...
distance = dijkstra(start_state, map_matrix)
print(distance)
