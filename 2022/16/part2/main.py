...
valve_set = frozenset([valve for valve in valves if valve.flow_rate > 0])

vals = []
for i in range(1, len(valve_set)):
    for valve_combination in itertools.combinations(valve_set, i):
        valve_combination = frozenset(valve_combination)
        maxval = dfs(TOTAL_MINUTES, starting_valve, valve_combination) \
                 + dfs(TOTAL_MINUTES, starting_valve, valve_set - valve_combination)
        vals.append(maxval)
