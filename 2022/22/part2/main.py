...
class Cell:
    ...
    def __init__(self, location: Tuple[int, int], mark: str):
        ...
        self.is_wall = mark == '#'
        self.is_valid = mark != ' '
        self.neighbours: Dict[Direction, Optional[Tuple['Cell', Direction]]] = {
    ...
    def set_neighbour(self, direction: Direction, cell: 'Cell', new_direction: Direction) -> None:
        self.neighbours[direction] = cell, new_direction

    def move(self, direction: Direction) -> Optional[Tuple['Cell', Direction]]:
        neighbour, new_direction = self.neighbours[direction]
        return neighbour, new_direction

    def __repr__(self) -> str:
        return f'{self.location}: {self.mark}'


# DIDNT DO IT CAUSE TEST AND INPUT CUBES DIFFER AND TASK IS COVERING EDGE CASES LMAO
...
# But basically, just change the way neighbours are calculated. That's all.
...
for instruction in instructions:
    ...
    if instruction in ('R', 'L'):
        ...
        for _ in range(move):
            next_cell, new_direction = current_cell.move(current_direction)
